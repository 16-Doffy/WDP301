...
export const buildFileUrl = (dataItem) => {
  ...
  if (cleanPath) {
    // path đã chứa full path + filename, dùng trực tiếp
  ...
  return (
    <div className="w-72 border-l border-gray-700 bg-gray-800 flex flex-col overflow-hidden">
      <div className="p-3 border-b border-gray-700">
        <h3 className="text-sm font-semibold text-gray-300">Annotations</h3>
      </div>
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {objects.length > 0 && (
          <div>
            <p className="text-xs text-gray-500 mb-1">Bounding Boxes ({objects.length})</p>
            {objects.map((obj, idx) => (
              <div key={idx} className="flex items-center justify-between rounded bg-gray-700/50 px-2 py-1 mb-1">
                <span className="text-xs text-gray-200">{obj.label || 'No label'}</span>
                {!readOnly && <button onClick={() => onLabelDelete('object', idx)} className="text-rose-400 hover:text-rose-300 text-xs">x</button>}
              </div>
            ))}
          </div>
        )}
        {textSpans.length > 0 && (
          <div>
            <p className="text-xs text-gray-500 mb-1">Text Spans ({textSpans.length})</p>
            {textSpans.map((span, idx) => (
              <div key={idx} className="flex items-center justify-between rounded bg-gray-700/50 px-2 py-1 mb-1">
                <span className="text-xs text-gray-200">"{span.text?.slice(0, 20)}..." - {span.label}</span>
                {!readOnly && <button onClick={() => onLabelDelete('span', idx)} className="text-rose-400 hover:text-rose-300 text-xs">x</button>}
              </div>
            ))}
          </div>
        )}
        {audioSegments.length > 0 && (
          <div>
            <p className="text-xs text-gray-500 mb-1">Audio Segments ({audioSegments.length})</p>
            {audioSegments.map((seg, idx) => (
              <div key={idx} className="flex items-center justify-between rounded bg-gray-700/50 px-2 py-1 mb-1">
                <span className="text-xs text-gray-200">{seg.label || 'No label'} [{seg.start?.toFixed(1)}s]</span>
                {!readOnly && <button onClick={() => onLabelDelete('segment', idx)} className="text-rose-400 hover:text-rose-300 text-xs">x</button>}
              </div>
            ))}
          </div>
        )}
      ...
      <div className="border-t border-gray-700 p-3">
        ...
        <textarea
          className="w-full rounded border border-gray-600 bg-gray-900 text-gray-200 p-2 text-xs resize-none focus:outline-none focus:border-blue-500"
          rows={3} value={note || ''} onChange={(e) => onNoteChange(e.target.value)}
          placeholder="Nhap ghi chu..." disabled={readOnly}
        />
      </div>
    </div>
  );
...
const TaskWrapper = () => {
  ...
  const [groupId, setGroupId] = useState(null); // subtopicId or datasetId
  ...
  const fetchTask = async () => {
    ...
    try {
      ...
      const groupKey = subtopicId || datasetId;
      setGroupId(groupKey);
      const tasksRes = await axios.get(`${API_URL}/api/tasks/my-tasks`, {
        params: groupKey ? { subtopicId: subtopicId || undefined, datasetId: subtopicId ? undefined : datasetId } : {},
      });
      setTasks(tasksRes.data || []);
    } catch (err) {
      console.error(err);
      setMessage('Loi tai task');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = useCallback(async () => {
    ...
    setMessage('');
    try {
      await axios.put(`${API_URL}/api/tasks/${id}/label`, { labels: buildLabelsPayload(), status: 'in_progress' });
      setMessage('Da luu!');
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      const nextTask = tasks.find((t, i) => i > currentIdx && t.status !== 'approved' && t.status !== 'submitted');
      if (nextTask) navigate(`/annotator/tasks/${nextTask._id}`);
      else setShowSubmitBatchDialog(true);
    } catch (err) {
  ...
  const handleSubmitBatch = async () => {
    try {
      await axios.post(`${API_URL}/api/tasks/submit-batch`, {
        subtopicId: task.subtopicId?._id || task.subtopicId || undefined,
        datasetId: task.subtopicId ? undefined : (task.datasetId?._id || task.datasetId),
      });
  ...
  const allDone = tasks.length > 0 && tasks.every((t) => t.status === 'completed' || t.status === 'submitted' || t.status === 'approved');
    ...
    <div className="flex flex-col h-screen bg-slate-900 text-gray-200">
      <div className="flex items-center justify-between border-b border-gray-700 bg-gray-800 px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {allDone && <Button size="small" variant="contained" color="success" onClick={() => setShowSubmitBatchDialog(true)}>Nop tat ca</Button>}
          {!allDone && completedCount > 0 && (
            <Button size="small" variant="contained" color="success" onClick={() => setShowSubmitBatchDialog(true)}>
              Nop {completedCount}/{tasks.length}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 overflow-auto bg-gray-900 p-4">
          ...
          {kind === 'audio' && <AudioPlayer task={task} segments={audioSegments} onChange={setAudioSegments} readOnly={isReadOnly} />}
          {kind === 'text' && <TextAnnotator task={task} spans={textSpans} onChange={setTextSpans} readOnly={isReadOnly} />}
      ...
      <Dialog open={showSubmitBatchDialog} onClose={() => setShowSubmitBatchDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Xac nhan nop bai</DialogTitle>
        <DialogContent>
          <Typography paragraph>Ban co {completedCount} task da hoan thanh trong tong so {tasks.length} task.</Typography>
          <Typography>Neu nop, cac task se duoc gui cho reviewer cham diem. Ban khong the chinh sua sau khi nop.</Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => { setShowSubmitBatchDialog(false); navigate('/annotator/tasks'); }}>Quay lai</Button>
          <Button onClick={handleSubmitBatch} variant="contained" color="success" disabled={saving}>{saving ? 'Dang nop...' : 'Xac nhan nap'}</Button>
        </DialogActions>
      </Dialog>
    </div>
  );
...
// --- Annotation components (placeholders) ---
const ImageCanvas = ({ task, annotations, onChange, readOnly }) => {
  const fileUrl = buildFileUrl(task?.dataItem);
  const [selectedLabel, setSelectedLabel] = useState('');
  const labels = task?.availableLabels || [];
  return (
    <div className="flex flex-col items-center">
      <p className="text-sm text-gray-400 mb-2">ImageCanvas - vung bbox tren anh (can implement)</p>
      {fileUrl && <img src={fileUrl} alt="task" className="max-w-full border border-gray-700 rounded" />}
      <div className="flex flex-wrap gap-2 mt-3">
        {labels.map((l) => (
          <button key={l.name} onClick={() => setSelectedLabel(l.name)}
            className={`px-3 py-1 rounded text-xs font-medium border transition ${selectedLabel === l.name ? 'ring-2 ring-blue-500' : 'opacity-70 hover:opacity-100'}`}
            style={{ borderColor: l.color || '#3b82f6', color: l.color || '#3b82f6', backgroundColor: selectedLabel === l.name ? l.color + '20' : 'transparent' }}>
            {l.name}
          </button>
        ))}
      </div>
...
const AudioPlayer = ({ task, segments, onChange, readOnly }) => {
  const fileUrl = buildFileUrl(task?.dataItem);
  return (
    <div className="flex flex-col items-center">
      <p className="text-sm text-gray-400 mb-2">AudioPlayer - phan tich am thanh (can implement wavesurfer)</p>
      {fileUrl && <audio controls src={fileUrl} className="mt-4" />}
      <p className="text-xs text-gray-500 mt-2">Segments: {segments.length}</p>
    </div>
  );
};

const TextAnnotator = ({ task, spans, onChange, readOnly }) => {
  ...
  useEffect(() => {
    if (task?.dataItem?.path) {
      const url = buildFileUrl(task.dataItem);
      axios.get(url, { responseType: 'text' }).then((r) => setContent(r.data)).catch(() => setContent(''));
    }
  }, [task]);
  ...
  const handleMouseUp = () => {
    ...
    const label = labels[0]?.name || 'label';
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div id="text-content" className="border border-gray-700 rounded p-4 whitespace-pre-wrap leading-relaxed select-text max-h-96 overflow-auto"
        style={{ backgroundColor: '#111827' }} onMouseUp={handleMouseUp}>
        {content || 'Dang tai noi dung...'}
      </div>
