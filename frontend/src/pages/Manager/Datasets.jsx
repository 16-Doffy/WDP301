...
const Datasets = () => {
  ...
  const handleOpenEdit = (ds) => {
    ...
    setFormData({ name: ds.name || '', description: ds.description || '', type: ds.type || 'image' });
    setEditDialogOpen(true);
  };

  const handleUpdateDataset = async () => {
    if (!editingDataset) return;
    if (!formData.name.trim()) return alert('Nhap ten dataset');
    setUpdatingDataset(true);
    try {
      await axios.put(API_URL + '/api/datasets/' + editingDataset._id, {
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3, md: 4 }, minHeight: '100vh', background: '#0f172a' }}>
      ...
      <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)} maxWidth="md" fullWidth PaperProps={{ sx: { bgcolor: '#1e293b', color: '#e2e8f0', border: '1px solid #334155' } }}>
        <DialogTitle sx={{ fontWeight: 700 }}>Edit Dataset</DialogTitle>
        <DialogContent>
          <TextField fullWidth label="Dataset Name *" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} margin="normal" />
          <TextField fullWidth multiline rows={3} label="Description" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} margin="normal" />
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 3 }}>
          <Button onClick={() => setEditDialogOpen(false)} sx={{ textTransform: 'none', fontWeight: 700, color: '#cbd5e1' }}>Huy</Button>
          <Button onClick={handleUpdateDataset} variant="contained" disabled={updatingDataset || !formData.name.trim()} sx={{ borderRadius: 2, px: 4, textTransform: 'none', fontWeight: 700, bgcolor: '#2563eb', '&:hover': { bgcolor: '#3b82f6' } }}>
            {updatingDataset ? <CircularProgress size={20} color="inherit" /> : 'Luu thay doi'}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
