...
const Datasets = () => {
  ...
  const handleFileUpload = (e) => {
    ...
    const isZip = (file) => file.name.toLowerCase().endsWith('.zip') || (file.type || '').includes('zip');
    ...
    // ZIP mode: only allow exactly one zip file
    ...
    if (files.some(isZip)) {
      ...
      if (files.length !== 1 || !isZip(files[0])) {
        ...
        alert('Nếu upload ZIP, vui lòng chỉ chọn đúng 1 file .zip.');
      ...
      if (files[0].size > 200 * 1024 * 1024) {
        ...
        alert(`File ZIP vượt quá 200MB: ${files[0].name}`);
  ...
  const getAcceptForType = (type) => {
    ...
    if (type === 'audio') return 'audio/*,.mp3,.wav,.m4a,.ogg,.mp4,.m4v,.zip,application/zip,application/x-zip-compressed';
    ...
    if (type === 'text') return '.txt,.csv,.json,.xml,.zip,text/plain,text/csv,application/json,application/xml,application/zip,application/x-zip-compressed';
            ...
            <>
              ...
              >
                ...
                <Typography variant="caption" sx={{ mt: 1, display: 'block', color: '#9ca3af' }}>
                  ...
                  {formData.type === 'audio'
                    ? 'Accepted: MP3, WAV, M4A, OGG, MP4, M4V hoặc 1 file ZIP'
                    : formData.type === 'text'
                      ? 'Accepted: TXT, CSV, JSON, XML hoặc 1 file ZIP'
                      : 'Accepted: JPG, PNG, GIF, BMP, WEBP hoặc 1 file ZIP'}
