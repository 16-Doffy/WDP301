    ...
    <Box sx={{ ...pageSx, p: { xs: 2, sm: 3, md: 4 } }}>
      ...
      <Dialog open={assignDialogOpen} onClose={() => setAssignDialogOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: modalPaperSx }} BackdropProps={{ sx: { backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)' } }}>
        ...
        <DialogContent sx={{ pt: 1 }}>
          <Stack spacing={2}>
            <FormControl fullWidth sx={inputSx}><InputLabel>Dataset</InputLabel><Select value={selectedDataset} label="Dataset" onChange={(e) => setSelectedDataset(e.target.value)}>{datasets.map((ds) => <MenuItem key={ds._id} value={ds._id}>{ds.name}</MenuItem>)}</Select></FormControl>
