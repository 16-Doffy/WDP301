...
$path = "d:\Desktop\WDP\WDP301\frontend\src\pages\Manager\Datasets.jsx"
...
const Datasets = () => {
  ...
  const loadSubtopicsForTopic = async (topicId) => {
    try {
      ...
      setSubtopics(Array.isArray(res) ? res : []);
    } catch { setSubtopics([]); }
  };
  ...
  const filtered = useMemo(() => {
    let r = [...datasets];
    if (search.trim()) { const t = search.toLowerCase(); r = r.filter(ds => (ds.name || '').toLowerCase().includes(t) || (ds.description || '').toLowerCase().includes(t)); }
    ...
    r.sort((a, b) => {
      ...
      else if (sortBy === 'progress') { const sg = id => { const s = statusByDs[id]; const raw = s?.totalRawItems || 0; const ap = s?.counts?.approved || 0; return raw > 0 ? (ap / raw) * 100 : 0; }; av = sg(a._id); bv = sg(b._id); }
    ...
    try {
      ...
      if (created?._id) {
        navigate('/manager/topics', {
          state: { highlightDsId: created._id, highlightDsName: created.name, selectedTopicId, selectedSubtopicId, autoTab: 'datasets' },
        });
      }
    } catch (err) {
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} maxWidth="md" fullWidth PaperProps={{ sx: modalSx }}>
        ...
        <DialogContent>
          ...
          <Box sx={{ p: 2, border: '1px solid #3b82f6', borderRadius: 2, mb: 2, bgcolor: 'rgba(59,130,246,0.05)' }}>
            ...
            {selectedSubtopicId && <Typography variant="caption" sx={{ color: '#475569', mt: 0.5, display: 'block', fontStyle: 'italic' }}>Subtopic context is linked. Label sets will be auto-inherited from this subtopic.</Typography>}
          </Box>
          {subtopicAssets ? (
              ...
              {subtopicLabels.length > 0 && (
                <Box sx={{ mt: 2 }}>
                  ...
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                    {subtopicLabels.map(ls => <Chip key={ls._id} label={ls.name} size="small" sx={{ bgcolor: 'rgba(59,130,246,0.15)', color: '#60a5fa', fontWeight: 600, fontSize: '0.7rem' }} />)}
                  </Box>
                </Box>
              )}
            </Box>
...
Write-Host "SUCCESS: Datasets.jsx written"
'@

Set-Content -Path "d:\Desktop\WDP\WDP301\frontend\src\pages\Manager\write_ds.ps1" -Value $ps -Encoding UTF8
& "d:\Desktop\WDP\WDP301\frontend\src\pages\Manager\write_ds.ps1"
