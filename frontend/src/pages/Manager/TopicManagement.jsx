...
import { Add as AddIcon, Edit as EditIcon, Delete as DeleteIcon, Folder as FolderIcon, Search as SearchIcon, Image as ImageIcon, Label as LabelIcon, LocalOffer as TagIcon, Upload as UploadIcon } from '@mui/icons-material';
...
const getImgUrl = (path) => { if (!path) return ''; const base = API_URL.replace(/\/+$/, ''); return base + '/' + path.replace(/^\/+/, ''); };

const AssetThumb = ({ asset, onDelete }) => {
  const url = getImgUrl(asset.path);
  const isImg = /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(asset.path || '');
  return (
    <Box sx={{ position: 'relative', borderRadius: 2, overflow: 'hidden', border: '1px solid #334155', '&:hover .del-btn': { opacity: 1 } }}>
      {isImg && url ? <img src={url} alt={asset.originalName || 'asset'} style={{ width: '100%', height: 80, objectFit: 'cover', display: 'block' }} /> : (
        <Box sx={{ width: '100%', height: 80, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: '#0f172a' }}><ImageIcon sx={{ color: '#64748b', fontSize: 32 }} /></Box>
      )}
      <Box className="del-btn" sx={{ position: 'absolute', top: 2, right: 2, opacity: 0, transition: 'opacity 0.2s' }}>
        <IconButton size="small" sx={{ bgcolor: 'rgba(0,0,0,0.6)', color: '#f87171', '&:hover': { bgcolor: 'rgba(239,68,68,0.3)' } }} onClick={() => onDelete(asset._id)}><DeleteIcon sx={{ fontSize: 14 }} /></IconButton>
      </Box>
    </Box>
  );
};
...
const LabelChip = ({ label, onDelete }) => (
  <Chip label={label.name} size="small" sx={{ bgcolor: label.color + '30', color: label.color, border: '1px solid ' + label.color, fontWeight: 600, mr: 0.5, mb: 0.5 }}
    deleteIcon={onDelete ? <DeleteIcon sx={{ fontSize: 14 }} /> : undefined}
    onDelete={onDelete ? () => onDelete(label.name) : undefined}
  />
);

const SubtopicPanel = ({ selectedSubtopic, onSubtopicUpdate, onCreateDataset }) => {
  ...
  useEffect(() => { if (selectedSubtopic) { loadLabelSets(); loadAssets(); loadDatasets(); } }, [selectedSubtopic?._id]);

  const loadLabelSets = async () => { setLabelSetsLoading(true); try { const r = await axios.get(API_URL + '/api/labelsets?subtopicId=' + selectedSubtopic._id, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); setLabelSets(Array.isArray(r.data) ? r.data : []); } catch { setLabelSets([]); } setLabelSetsLoading(false); };
  const loadDatasets = async () => { try { const r = await axios.get(API_URL + '/api/datasets?subtopicId=' + selectedSubtopic._id, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); setDatasets(Array.isArray(r.data) ? r.data : []); } catch { setDatasets([]); } };
  const loadAssets = async () => { setAssetsLoading(true); try { const r = await axios.get(API_URL + '/api/subtopics/' + selectedSubtopic._id + '/assets', { headers: { Authorization: 'Bearer ' + getAuthToken() } }); setAssets(Array.isArray(r.data) ? r.data : []); } catch { setAssets([]); } setAssetsLoading(false); };

  const saveLabelSet = async () => { try { if (labelDialog.edit) await axios.put(API_URL + '/api/labelsets/' + labelDialog.data._id, labelDialog.data, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); else await axios.post(API_URL + '/api/labelsets', { ...labelDialog.data, subtopicId: selectedSubtopic._id }, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); setLabelDialog({ open: false, edit: false, data: { name: 'Default', labels: [], allowMultiple: false } }); loadLabelSets(); setSnackbar({ open: true, message: 'Luu labelset thanh cong!', severity: 'success' }); } catch (e) { setSnackbar({ open: true, message: 'Loi: ' + e.message, severity: 'error' }); } };
  ...
  const handleUploadAssets = async () => { if (uploadFiles.length === 0) return; setUploading(true); try { const fd = new FormData(); uploadFiles.forEach(f => fd.append('files', f)); await axios.post(API_URL + '/api/subtopics/' + selectedSubtopic._id + '/assets', fd, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); setUploadFiles([]); loadAssets(); onSubtopicUpdate(); setSnackbar({ open: true, message: 'Upload thanh cong!', severity: 'success' }); } catch (e) { setSnackbar({ open: true, message: 'Loi: ' + (e.response?.data?.message || e.message), severity: 'error' }); } setUploading(false); };
  const handleDeleteAsset = async (assetId) => { if (!confirm('Xoa asset?')) return; try { await axios.delete(API_URL + '/api/subtopics/' + selectedSubtopic._id + '/assets/' + assetId, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); loadAssets(); onSubtopicUpdate(); } catch (e) { setSnackbar({ open: true, message: 'Loi: ' + e.message, severity: 'error' }); } };
  ...
  if (!selectedSubtopic) return (
    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: 400 }}>
      <Box sx={{ textAlign: 'center' }}><FolderIcon sx={{ fontSize: 64, color: '#334155', mb: 2 }} /><Typography variant="h6" sx={{ color: '#475569' }}>Chon 1 subtopic de xem chi tiet</Typography></Box>
    </Box>
  );

  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <Box sx={{ p: 2, borderBottom: '1px solid #334155' }}><Typography variant="h6" fontWeight={700} sx={{ color: '#e2e8f0' }}>Subtopic Detail: {selectedSubtopic.name}</Typography></Box>
      <Box sx={{ flex: 1, overflow: 'auto', p: 2 }}>
        <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
          {[{ icon: <ImageIcon />, label: 'Assets', value: totalAssets, color: '#3b82f6' }, { icon: <TagIcon />, label: 'Labels', value: totalLabels, color: '#22c55e' }, { icon: <FolderIcon />, label: 'Datasets', value: totalDatasets, color: '#f59e0b' }].map(s => (
            <Box key={s.label} sx={{ flex: 1, p: 2, borderRadius: 2, bgcolor: '#0f172a', border: '1px solid #334155', display: 'flex', alignItems: 'center', gap: 1.5 }}>
              ...
              <Box><Typography variant="h6" fontWeight={800} sx={{ color: '#e2e8f0', lineHeight: 1 }}>{s.value}</Typography><Typography variant="caption" sx={{ color: '#64748b' }}>{s.label}</Typography></Box>
            </Box>
          ))}
        </Box>
        <Grid container spacing={3}>
            ...
            <Paper sx={{ ...panelSx, p: 2 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  ...
                  <Button size="small" variant="outlined" startIcon={<FolderIcon />} onClick={onCreateDataset} sx={{ ...btnOutline, py: 0.5, px: 1.5, fontSize: '0.75rem' }}>Dataset</Button>
                </Box>
                <input ref={fileInputRef} type="file" multiple style={{ display: 'none' }} onChange={handleFileSelect} accept="image/*,audio/*,text/*" />
              </Box>
              {uploadFiles.length > 0 && (
                <Box sx={{ mb: 2, p: 2, borderRadius: 2, bgcolor: '#0f172a', border: '1px dashed #3b82f6' }}>
                  ...
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mb: 1 }}>{uploadFiles.map((f, i) => <Chip key={i} label={f.name} size="small" onDelete={() => setUploadFiles(prev => prev.filter((_, idx) => idx !== i))} sx={{ bgcolor: '#334155', color: '#e2e8f0' }} />)}</Box>
                  <Button size="small" variant="contained" onClick={handleUploadAssets} disabled={uploading} sx={btnPrimary}>{uploading ? 'Dang upload...' : 'Upload'}</Button>
                </Box>
              )}
              ...
              {assetsLoading ? <CircularProgress size={24} sx={{ display: 'block', mx: 'auto', mt: 4 }} /> : filteredAssets.length === 0 ? (
                <Box sx={{ textAlign: 'center', py: 6, border: '2px dashed #334155', borderRadius: 2, cursor: 'pointer', bgcolor: dragOver ? 'rgba(59,130,246,0.1)' : 'transparent', borderColor: dragOver ? '#3b82f6' : '#334155' }}
                  onClick={() => fileInputRef.current?.click()} onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={handleDrop}>
                  <UploadIcon sx={{ fontSize: 40, color: '#475569', mb: 1 }} /><Typography variant="body2" sx={{ color: '#64748b' }}>Drop files here or click Upload</Typography>
                </Box>
              ) : (
                <Grid container spacing={1}>{filteredAssets.map(a => <Grid item xs={4} sm={3} key={a._id}><AssetThumb asset={a} onDelete={handleDeleteAsset} /></Grid>)}</Grid>
              )}
            </Paper>
          </Grid>
          <Grid item xs={12} md={5}>
            <Paper sx={{ ...panelSx, p: 2 }}>
              ...
              {labelSetsLoading ? <CircularProgress size={24} sx={{ display: 'block', mx: 'auto', mt: 4 }} /> : labelSets.length === 0 ? (
                <Box sx={{ textAlign: 'center', py: 4 }}><TagIcon sx={{ fontSize: 40, color: '#334155', mb: 1 }} /><Typography variant="body2" sx={{ color: '#64748b' }}>Chua co labelset nao</Typography></Box>
              ) : labelSets.map(ls => (
                <Box key={ls._id} sx={{ mb: 2 }}>
                  ...
                  <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>{(ls.labels || []).map(l => <LabelChip key={l.name} label={l} onDelete={removeLabel} />)}</Box>
                </Box>
              ))}
            </Paper>
          </Grid>
      ...
      <Dialog open={labelDialog.open} onClose={() => setLabelDialog({ ...labelDialog, open: false })} maxWidth="sm" fullWidth PaperProps={{ sx: modalSx }}>
        ...
        <DialogContent>
          ...
          <Box sx={{ mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', mb: 1 }}>{(labelDialog.data.labels || []).map(l => <Chip key={l.name} label={l.name} size="small" sx={{ bgcolor: l.color + '30', color: l.color, border: '1px solid ' + l.color, mr: 0.5, mb: 0.5 }} onDelete={() => removeLabel(l.name)} deleteIcon={<DeleteIcon sx={{ fontSize: 14 }} />} />)}</Box>
            <Box sx={{ display: 'flex', gap: 1 }}>
              ...
              <Button onClick={addLabel} variant="outlined" sx={{ ...btnOutline, py: 0.5 }}>Add</Button>
            </Box>
          </Box>
        </DialogContent>
      ...
      <Snackbar open={snackbar.open} autoHideDuration={3000} onClose={() => setSnackbar({ ...snackbar, open: false })}><Alert severity={snackbar.severity} sx={{ bgcolor: '#1e293b', color: '#e2e8f0' }}>{snackbar.message}</Alert></Snackbar>
    </Box>
  );
};
...
const TopicManagement = () => {
  ...
  const [createDatasetDialog, setCreateDatasetDialog] = useState({ open: false, data: { name: '', taskType: 'classification' } });
  const [snackbar, setSnackbar] = useState({ open: false, message: '', severity: 'success' });

  useEffect(() => { loadTopics(); }, []);

  const loadTopics = async () => { setLoading(true); try { const r = await axios.get(API_URL + '/api/topics', { headers: { Authorization: 'Bearer ' + getAuthToken() } }); setTopics(Array.isArray(r.data) ? r.data : []); } catch { setTopics([]); } setLoading(false); };
  const loadSubtopicsForTopic = async (topic) => { try { const r = await axios.get(API_URL + '/api/subtopics?topicId=' + topic._id, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); return Array.isArray(r.data) ? r.data : []; } catch { return []; } };

  const saveTopic = async () => { try { if (topicDialog.edit) await axios.put(API_URL + '/api/topics/' + selectedTopic._id, topicDialog.data, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); else await axios.post(API_URL + '/api/topics', topicDialog.data, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); setTopicDialog({ ...topicDialog, open: false }); loadTopics(); setSnackbar({ open: true, message: topicDialog.edit ? 'Cap nhat topic thanh cong!' : 'Tao topic thanh cong!', severity: 'success' }); } catch (e) { setSnackbar({ open: true, message: 'Loi: ' + e.message, severity: 'error' }); } };
  const handleDeleteTopic = async (id) => { if (!confirm('Xoa topic nay?')) return; try { await axios.delete(API_URL + '/api/topics/' + id, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); if (selectedTopic?._id === id) { setSelectedTopic(null); setSelectedSubtopic(null); } loadTopics(); } catch (e) { alert('Loi: ' + e.message); } };

  const saveSubtopic = async () => { if (!selectedTopic) return; try { if (subtopicDialog.edit) await axios.put(API_URL + '/api/subtopics/' + selectedSubtopic._id, subtopicDialog.data, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); else await axios.post(API_URL + '/api/subtopics', { ...subtopicDialog.data, topicId: selectedTopic._id }, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); setSubtopicDialog({ ...subtopicDialog, open: false }); const subs = await loadSubtopicsForTopic(selectedTopic); setExpandedTopic({ ...selectedTopic, subtopics: subs }); setSelectedSubtopic(null); loadTopics(); setSnackbar({ open: true, message: subtopicDialog.edit ? 'Cap nhat subtopic thanh cong!' : 'Tao subtopic thanh cong!', severity: 'success' }); } catch (e) { setSnackbar({ open: true, message: 'Loi: ' + e.message, severity: 'error' }); } };
  const handleDeleteSubtopic = async (id) => { if (!confirm('Xoa subtopic nay?')) return; try { await axios.delete(API_URL + '/api/subtopics/' + id, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); if (selectedSubtopic?._id === id) setSelectedSubtopic(null); if (expandedTopic) { const subs = await loadSubtopicsForTopic(expandedTopic); setExpandedTopic({ ...expandedTopic, subtopics: subs }); } loadTopics(); } catch (e) { alert('Loi: ' + e.message); } };

  const handleCreateDataset = async () => { if (!createDatasetDialog.data.name.trim()) { setSnackbar({ open: true, message: 'Nhap ten dataset', severity: 'error' }); return; } try { await axios.post(API_URL + '/api/datasets', { ...createDatasetDialog.data, subtopicId: selectedSubtopic._id }, { headers: { Authorization: 'Bearer ' + getAuthToken() } }); setCreateDatasetDialog({ open: false, data: { name: '', taskType: 'classification' } }); setSnackbar({ open: true, message: 'Tao dataset thanh cong!', severity: 'success' }); } catch (e) { setSnackbar({ open: true, message: 'Loi: ' + e.message, severity: 'error' }); } };

  const handleTopicClick = async (topic) => { if (selectedTopic?._id === topic._id) { setSelectedTopic(null); setSelectedSubtopic(null); setExpandedTopic(null); return; } setSelectedTopic(topic); setSelectedSubtopic(null); const subs = await loadSubtopicsForTopic(topic); setExpandedTopic({ ...topic, subtopics: subs }); };
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3, md: 4 }, minHeight: '100vh', bgcolor: '#0f172a' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box><Typography variant="h4" fontWeight={800} sx={{ color: '#e2e8f0', mb: 0.5 }}>Topic Management</Typography><Typography variant="body2" sx={{ color: '#94a3b8' }}>Quan ly chu de lon va chu de nho</Typography></Box>
      ...
      <Grid container spacing={3} sx={{ minHeight: 'calc(100vh - 220px)' }}>
                ...
                <Box key={t._id} sx={{ mb: 1 }}>
                  ...
                  {selectedTopic?._id === t._id && expandedTopic && (
                    <Box sx={{ pl: 3, mt: 1 }}>
                      ...
                      {(!expandedTopic.subtopics || expandedTopic.subtopics.length === 0) ? <Typography variant="caption" sx={{ color: '#64748b', pl: 1 }}>Chua co subtopic</Typography> : expandedTopic.subtopics.map(s => (
        ...
        <Grid item xs={12} md={selectedSubtopic ? 8 : 9}>
          <Paper sx={{ ...panelSx, height: '100%' }}>
            <SubtopicPanel selectedSubtopic={selectedSubtopic} onSubtopicUpdate={loadTopics} onCreateDataset={() => setCreateDatasetDialog({ open: true, data: { name: '', taskType: 'classification' } })} />
          </Paper>
        </Grid>
      </Grid>
      ...
      <Dialog open={createDatasetDialog.open} onClose={() => setCreateDatasetDialog({ ...createDatasetDialog, open: false })} maxWidth="sm" fullWidth PaperProps={{ sx: modalSx }}>
        <DialogTitle sx={{ fontWeight: 700 }}>Create New Dataset</DialogTitle>
        <DialogContent>
          <Typography variant="body2" sx={{ color: '#64748b', mb: 2, mt: 1 }}>Subtopic: <strong style={{ color: '#e2e8f0' }}>{selectedSubtopic?.name}</strong></Typography>
          <TextField fullWidth label="Dataset Name" placeholder="Enter dataset name" value={createDatasetDialog.data.name} onChange={e => setCreateDatasetDialog({ ...createDatasetDialog, data: { ...createDatasetDialog.data, name: e.target.value } })} sx={{ mb: 2, ...inputSx }} />
          <FormControl fullWidth sx={{ mb: 2, ...inputSx }}>
            <InputLabel>Dataset Type</InputLabel>
            <Select value={createDatasetDialog.data.taskType} label="Dataset Type" onChange={e => setCreateDatasetDialog({ ...createDatasetDialog, data: { ...createDatasetDialog.data, taskType: e.target.value } })}>
              <MenuItem value="classification">Image Classification</MenuItem>
              <MenuItem value="bbox">Object Detection (BBox)</MenuItem>
              <MenuItem value="text">Text Classification</MenuItem>
              <MenuItem value="audio">Audio Classification</MenuItem>
            </Select>
          </FormControl>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setCreateDatasetDialog({ ...createDatasetDialog, open: false })} sx={btnSecondary}>Cancel</Button>
          <Button variant="contained" onClick={handleCreateDataset} sx={btnPrimary}>Create Dataset</Button>
        </DialogActions>
      </Dialog>
      <Snackbar open={snackbar.open} autoHideDuration={3000} onClose={() => setSnackbar({ ...snackbar, open: false })}><Alert severity={snackbar.severity} sx={{ bgcolor: '#1e293b', color: '#e2e8f0' }}>{snackbar.message}</Alert></Snackbar>
    </Box>
