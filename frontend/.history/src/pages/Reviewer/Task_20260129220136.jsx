...
const ReviewerTask = () => {
  ...
  const handleReject = useCallback(async () => {
    ...
    if (window.confirm('Are you sure you want to reject this task? The annotator will need to re-work it.')) {
      ...
      try {
        ...
        // If autoNext is enabled, attempt to fetch the next task id and navigate there.
        if (autoNext) {
          try {
            const nextResp = await axios.get(`${API_URL}/api/tasks/${id}/next`);
            const nextId = nextResp.data?.id;
            if (nextId) {
              navigate(`/reviewer/tasks/${nextId}`);
              return;
            }
          } catch (err) {
            // fallback to list if next fetch fails
  ...
  }, [id, reviewComments, reviewNotes, errorCategory, navigate, autoNext]);
              ...
              <div className="max-w-6xl mx-auto flex items-center gap-8">
                ...
                <div className="flex items-center gap-4">
                    <button
                      ...
                      className="h-14 px-8 bg-rose-600 text-white rounded-[1.5rem] font-black text-xs uppercase tracking-widest hover:bg-rose-700 active:scale-95 disabled:opacity-30 disabled:grayscale transition-all flex items-center gap-3 shadow-sm"
                    >
                      <CloseIcon /> Reject
                    </button>
                    <button
