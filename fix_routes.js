...
const d='d:/Desktop/WDP/WDP301/backend/routes';

// topics.js - fixed $in syntax
const topics=`const express = require('express');
...
// subtopics.js
const subtopics=`const express = require('express');
const router = express.Router();
const Subtopic = require('../models/Subtopic');
const LabelSet = require('../models/LabelSet');
const { auth, authorize } = require('../middleware/auth');
...
// labelsets.js
const labelsets=`const express = require('express');
const router = express.Router();
const LabelSet = require('../models/LabelSet');
const { auth, authorize } = require('../middleware/auth');
...
fs.writeFileSync(path.join(d,'topics.js'),topics);
fs.writeFileSync(path.join(d,'subtopics.js'),subtopics);
fs.writeFileSync(path.join(d,'labelsets.js'),labelsets);
console.log('All routes written: topics('+topics.length+') subtopics('+subtopics.length+') labelsets('+labelsets.length+')');
